...
void display_move_away() {
  ...
  display.setFont();
  display.setTextSize(1); 
  display.print(F("You are too close."));
  display.display();
  }
...
void display_temp(const String target) {
  display.clearDisplay();
  display.drawRect(0, 0, 128, 64, SSD1306_WHITE);
  display.setTextSize(2);  
  display.setTextColor(SSD1306_WHITE);
  ...
  display.drawCircle(89, 30, 3, WHITE);
  display.display();
...
void display_splash(void) {
  ...
  display.print(F("BluePhoenix"));
  display.setCursor(33,76-i); 
  display.setFont(&FreeSans9pt7b);            
  display.print(F("AI.Can!"));
  display.drawLine(63-2*i, 32 , 65+2*i, 32,SSD1306_WHITE );
  display.display();
  delay(10);
  }
  delay(2500); 
